...
- A shared skill taxonomy on courses ([04-learner-dashboard.md](04-learner-dashboard.md)'s Retention card) — `courses.focus_area` is a per-course description, not a groupable skill; "Confidence by skill" has no real data to bucket by until one exists
