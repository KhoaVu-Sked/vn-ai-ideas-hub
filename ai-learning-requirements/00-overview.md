...
**Status:** In progress — Learning Hub, Your Journey (including the wrap-up quiz and Auto Schedule), a Learner Dashboard (rebuilt to follow a design mockup — KPI row, Learning/Consistency/Retention/What's next cards, a Weekly streak tied to Auto Schedule, and a shared course-skill taxonomy behind the Retention card's "Confidence by skill"), and Team view (also rebuilt to follow that same mockup's Team View tab — a KPI row, a real "Needs support" list, Level→target/Pace/Avg exam columns on the roster, a skills heatmap, and a level-distribution chart) are built and live in this repo. The Planner and Knowledge Builder agents are not built; Scheduler is split — Auto Schedule itself is real (deterministic, not Claude-driven), reminders and preference-aware sequencing are not. Two Learner Dashboard cards and Team view's "Ideas shipped"/Application card are explicit Phase 2 placeholders pending an idea↔course link on the Ideas Hub side (04-learner-dashboard.md, 05-team-view.md).
**Owner:** The Kiet
...
| `/learning-hub/team` — admin KPI row, Needs support, the roster (Level→target/Pace/Avg exam), the skills heatmap, distribution, read-only drill-down, the annual review date editor | [05-team-view.md](05-team-view.md) | ✅ Built (Ideas shipped/Application card Phase 2) |
