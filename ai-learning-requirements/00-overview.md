...
**Status:** In progress — Learning Hub (including a first-time "Get Started" onboarding gateway — role, optional Calendar connect, browse/enroll), Your Journey (including the wrap-up quiz and Auto Schedule), a Learner Dashboard (rebuilt to follow a design mockup — KPI row, Learning/Consistency/Retention/What's next/Application cards, a Weekly streak tied to Auto Schedule, and a shared course-skill taxonomy behind the Retention card's "Confidence by skill"), and Team view (also rebuilt to follow that same mockup's Team View tab — a KPI row, a real "Needs support" list, Level→target/Pace/Avg exam columns on the roster, a skills heatmap, a level-distribution chart, and an Application funnel) are built and live in this repo. The Planner and Knowledge Builder agents are not built; Scheduler is split — Auto Schedule itself is real (deterministic, not Claude-driven), reminders and preference-aware sequencing are not. Both Application cards (Dashboard and Team view) read the Ideas Hub's `ideas` table by owner + status only (`initiator_account_id`/`status`) — a real, deliberate cross-feature read, not a placeholder; the finer idea↔course/skill link (which course or skill a given idea came from) is still genuinely missing (04-learner-dashboard.md, 05-team-view.md, 09-out-of-scope.md).
**Owner:** The Kiet
...
|---|---|---|
| The course/quiz catalog, seeding (`ai-track-seed.sql`, `career-track-seed.sql`), or the (unbuilt) Sheets-sync idea | [01-course-catalog.md](01-course-catalog.md) | ✅ catalog data · ⬜ sync |
...
|---|---|---|
| Application data (accounts, tracks, courses, progress) | **Neon Postgres** — the same database as the rest of this app (`vn-ai-ideas-hub`), not a separate Supabase project. Both Application cards also read the Ideas Hub's own `ideas` table (owner + status only — see [08-data-model.md](08-data-model.md)), the one place this feature reads a table it doesn't own | ✅ Built |
| Course catalog + quiz content | Lives in `courses` and `course_quiz_questions`, across three tracks (AI Track, Career Track, and the still-empty Core Competency): the AI Track's 20 real courses (imported from a roadmap spreadsheet) plus quiz questions for 15 of them (imported from a separate course-framework spreadsheet), seeded via **`ai-track-seed.sql`**; and the Career Track's 36 real resources (the Intern/P1 slice of a separate career-ladder spreadsheet, no quiz content), seeded via its own sibling file, **`career-track-seed.sql`** — split into two files so either track's content can be edited and re-run independently. Both are standalone, idempotent, run-once-by-hand files, same pattern as `seed.sql`, kept out of `schema.sql` so that file stays pure table design. No live Google Sheets integration, no sync button, no editorial workflow for non-technical editors — see [01-course-catalog.md](01-course-catalog.md) | ✅ Built (as data), ⬜ (as an editable catalog workflow) |
| Wrap-up quiz | A learner opens a course's quiz (`/learning/journey/[courseId]/quiz`), clicks any option to check it against the stored answer — no locking, no attempt limit, no per-click history recorded. Finishing the last question marks the course `complete` and snapshots question count + first-try accuracy onto `course_assignments` — see [03-your-journey.md](03-your-journey.md) | ✅ Built |
| Web app / dashboards | **Next.js on Vercel**, same deployment as the Ideas Hub. Five pages: `/learning`, `/learning/journey`, `/learning/journey/[courseId]/quiz`, `/learning/dashboard`, `/learning/team` | ✅ Built |
...
**What changed from the original plan, and why:** the original architecture spread data across Google Sheets (catalog) and Supabase (progress), synced by a Vercel route. In practice, this got built as one Neon Postgres database shared with the existing Ideas Hub app — simpler to operate, and the catalog is small enough (56 courses across two tracks today) that a one-time SQL import was faster to ship than building a Sheets-sync pipeline. That pipeline ([01-course-catalog.md](01-course-catalog.md), 2.3) is not built and isn't currently planned; if the catalog grows past what's comfortable to edit via SQL migrations, that's the point to revisit it. The catalog content itself now lives in its own seed files (`ai-track-seed.sql`, `career-track-seed.sql` — one per track, [01-course-catalog.md](01-course-catalog.md)) rather than inline in `schema.sql`, for the same reason `seed.sql` is separate: table design and content are different kinds of change, run at different times.
