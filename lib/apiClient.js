...
// Anything that needs to reset when the session dies mid-request. api() hangs
// deliberately on a 401, so a caller counting in-flight writes would otherwise
// stay stuck above zero forever.
const onEnd = new Set();
export const onSessionEnd = (fn) => { onEnd.add(fn); return () => onEnd.delete(fn); };

export function endSession(reason = "ended") {
  if (leaving) return;              // parallel 401s must not each navigate
  leaving = true;
  for (const fn of onEnd) { try { fn(); } catch {} }
