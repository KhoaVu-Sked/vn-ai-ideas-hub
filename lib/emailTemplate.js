...
export function renderEmailText({ heading, intro, rows = [], quote, ctaLabel, ctaUrl, footer }) {
  const out = [heading, "", stripTags(intro), ""];
  rows.forEach(([k, v]) => out.push(`${k}: ${v}`));
  if (rows.length) out.push("");
  if (quote) out.push(quote, "");
  if (ctaUrl) out.push(`${ctaLabel || "Open"}: ${ctaUrl}`, "");
  out.push("—", footer || "You're receiving this because you follow or work on this idea in AI Ideas Hub.");
  return out.join("\n");
}

// rows: [[label, value], …] rendered as a small definition table (optional)
export function renderEmail({ heading, intro, rows = [], quote, ctaLabel, ctaUrl, footer }) {
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" style="width:100%;max-width:560px;background:#ffffff;border:1px solid ${LINE};border-radius:14px;overflow:hidden">
      <tr>
        <td style="background:${BLUE};padding:16px 24px">
          <span style="font:700 16px/1 Arial,Helvetica,sans-serif;color:#ffffff;letter-spacing:-0.01em">AI Ideas Hub</span>
        </td>
      </tr>
      ...
      <tr><td style="border-top:1px solid ${LINE};padding:14px 24px">
        <p style="margin:0;font:400 11.5px/1.5 Arial,Helvetica,sans-serif;color:${MUTED}">${esc(footer || "You're receiving this because you follow or work on this idea in AI Ideas Hub.")}</p>
      </td></tr>
    </table>
  </td></tr>
</table>
