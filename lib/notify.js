...
export function buildIdeaEmail({ meta, actor = "Someone", kind, detail = "", body = "", link }) {
  ...
  if (kind === "status") {
    subject = `[AI Ideas Hub] ${name} → ${detail.to}`;
    ...
    subject = `[AI Ideas Hub] ${name} was updated`;
    heading = "Idea updated";
    intro = `<b>${actor}</b> edited <b>${name}</b>.`;
    rows = [["Idea", `${meta.number} · ${name}`], ["Updated", detail || "Content"], ["Edited by", actor]];
  } else {
    subject = `[AI Ideas Hub] Update on ${name}`;
    heading = "Idea updated";
    intro = `There's an update on <b>${name}</b>.`;
  }
  return { subject, html: renderEmail({ heading, intro, rows, quote, ctaLabel: "View the idea", ctaUrl: link }) };
}
...
export async function notifyIdea(ideaId, { actorId, actor = "Someone", kind, detail = "", body = "", base } = {}) {
  try {
    ...
    const { subject, html } = buildIdeaEmail({ meta, actor, kind, detail, body, link: `${appUrl(base)}/idea/${ideaId}` });
    await sendEmail({ subject, to: recipients, html });
  } catch (e) {
