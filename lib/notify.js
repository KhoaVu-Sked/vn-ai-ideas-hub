...
export function buildIdeaEmail({ meta, actor = "Someone", kind, detail = "", body = "", link }) {
  ...
  const parts = {
    ...
    footer: `You're receiving this because you're a member or follower of ${meta.number} in AI Ideas Hub, Skedulo's internal AI ideas tracker. To stop these, reply with "unsubscribe".`,
  };
...
export async function notifyAdmins({ actorId, subject, heading, intro, rows = [], quote, ctaPath, base } = {}) {
  try {
    ...
    const parts = {
      ...
      ctaUrl: url,
      footer: `You're receiving this because you're an admin of AI Ideas Hub, Skedulo's internal AI ideas tracker. To stop these, reply with "unsubscribe".`,
    };
