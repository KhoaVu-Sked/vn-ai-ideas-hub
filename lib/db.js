...
// Add a comment, attributed to the signed-in user (falls back to 'Anonymous').
export async function addComment(id, text, author = "Anonymous") {
  const body = (text || "").trim().slice(0, 1800);
  const who = (author || "Anonymous").slice(0, 120);
  const rows = await sql`
    insert into comments (idea_id, body, author)
    values (${id}, ${body}, ${who})
    returning id, created_at
  `;
  return { id: rows[0].id, text, author: who, date: ymd(rows[0].created_at) };
}

// Look up an account by username (for login). Returns the row or undefined.
