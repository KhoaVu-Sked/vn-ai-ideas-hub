...
// One of each per idea, enforced by partial unique indexes.
...
// Nobody can join a team AS the initiator — that would be claiming to have
// raised an idea they didn't. Admins set it via the role editor.
export const JOINABLE_ROLES = ROLES.filter((r) => r !== INITIATOR_ROLE);
const REQUEST_STATES = ["open", "accepted", "under_discussion", "declined", "closed"];
...
function cleanRoles(roles, allowed = ROLES) {
  const list = (Array.isArray(roles) ? roles : [roles]).filter((r) => allowed.includes(r));
  if (list.length === 0) throw err(400, "Pick at least one role.");
  return [...new Set(list)];
}

export async function joinTeam(ideaId, accountId, roles) {
  // JOINABLE_ROLES, not ROLES: Initiator is a record of who raised the idea and
  // is never self-assignable. Joining keeps any Initiator you already hold.
  const list = cleanRoles(roles, JOINABLE_ROLES);
  try {
    const rows = await sql`
      with ins as (
        insert into idea_members (idea_id, account_id, roles)
        values (${ideaId}, ${accountId}, ${list})
        on conflict (idea_id, account_id) do update
          -- Carry Initiator across: it isn't in the submitted list (it can't be
          -- self-assigned), so a plain overwrite would silently drop it.
          set roles = case when idea_members.roles @> array['Initiator']
                           then array['Initiator'] || excluded.roles
