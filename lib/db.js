...
export async function createFormField({ label, type, options, required }) {
  ...
  await sql`insert into form_fields (key, label, type, options, required, position) values (${key}, ${lab}, ${t}, ${opts}, ${!!required}, ${pos})`;
...
export async function createProject({ name, tags, context, pain_points, expected_benefit, target_date, extra, initiatorAccountId }) {
  ...
  if (initiatorAccountId) {
    await sql`
      ...
      values (${idea.id}, ${initiatorAccountId}, 'Initiator / Project Lead')
...
export async function updateContent(id, { context, pain_points, expected_benefit, target_date, tags, extra }) {
  ...
  const rows = await sql`
    update ideas set
      ...
      tags = coalesce(${tagList}::text[], tags),
      extra = coalesce(extra, '{}'::jsonb) || coalesce(${extraJson}::jsonb, '{}'::jsonb),
      updated_at = now()
    where id = ${id}
    returning id
  `;
  ...
  const rows = await sql`
    ...
    where at.id = ${attId}
      and ( at.account_id = ${accountId}
         or ${isAdmin}
         or exists (select 1 from idea_members m where m.idea_id = at.idea_id and m.account_id = ${accountId} and m.role = 'Initiator / Project Lead') )
    returning url
  `;
...
export async function isProjectLead(ideaId, accountId) {
  const rows = await sql`
    ...
    where idea_id = ${ideaId} and account_id = ${accountId} and role = 'Initiator / Project Lead'
  `;
  ...
  const rows = await sql`
    ...
    where r.id = ${reqId}
      and ( r.account_id = ${accountId}
         or ${isAdmin}
         or exists (select 1 from idea_members m where m.idea_id = r.idea_id and m.account_id = ${accountId} and m.role = 'Initiator / Project Lead') )
    returning id
  `;
...
// Admin sets another member's role. Assigning the lead role transfers it:
// any existing lead on that idea is demoted to Observer in the same statement
// (the two updates touch disjoint rows, so this is safe).
export async function setMemberRole(ideaId, accountId, role) {
  if (!ROLES.includes(role)) throw err(400, "Invalid role.");
  const rows = await sql`
    with demote as (
      update idea_members set role = 'Observer'
      where idea_id = ${ideaId} and role = ${LEAD_ROLE} and account_id <> ${accountId}
        and ${role} = ${LEAD_ROLE}
      returning account_id
    )
    update idea_members m set role = ${role}
    where m.idea_id = ${ideaId} and m.account_id = ${accountId}
    returning m.account_id, m.role, (select count(*) from demote)::int as demoted
  `;
  if (rows.length === 0) throw err(404, "That person isn't on this idea's team.");
  return { account_id: rows[0].account_id, role: rows[0].role, demoted: rows[0].demoted > 0 };
}
...
export async function getDashboard({ since = null, quarterStart = null } = {}) {
  ...
  const funnel = order.map((s, i) => ({ stage: label[s], status: s, count: order.slice(i).reduce((n, st) => n + (byStatus[st] || 0), 0) }));
  ...
  `).map((c) => ({ tag: c.tag, count: c.n, color: c.color || null }));
...
export async function updateTask(id, { title, done }) {
  const rows = await sql`
    update tasks set
      ...
      done_at = case when ${typeof done === "boolean" ? done : null} = true then now()
                     when ${typeof done === "boolean" ? done : null} = false then null
                     else done_at end
    where id = ${id}
    returning id, title, done, created_at
  `;
