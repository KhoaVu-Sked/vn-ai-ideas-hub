...
export async function addTimeFrame(name) {
  ...
  await sql`insert into time_frames (name, position) values (${clean}, ${pos}) on conflict (name) do nothing`;
...
export async function moveFormField(id, direction) {
  ...
  `;
  await sql`
    update form_fields set position = case when id = ${active[i].id} then ${j + 1} else ${i + 1} end
    where id in (${active[i].id}, ${active[j].id})
  `;
  return listFormFields();
}
...
export async function updateStatus(id, status) {
  ...
  const members = await sql`
    ...
    from idea_members m join accounts a on a.id = m.account_id
    where m.idea_id = ${id} and exists (select 1 from unnest(m.roles) r where r <> 'Observer')
  `;
...
export async function getDashboard({ since = null, quarterStart = null } = {}) {
  ...
  const label = { "Submitted": "Submitted", "In Review": "In Review", "Approved": "Approved", "In Progress": "In Progress", "Pilot": "Pilot", "Launched": "Launched" };
  const funnel = order.map((s) => ({ stage: label[s], status: s, count: byStatus[s] || 0 }));
