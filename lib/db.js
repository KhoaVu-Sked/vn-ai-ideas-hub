...
export async function getIdea(id, accountId) {
  const rows = await sql`
    select i.id, i.name, i.status, i.tags, i.target_date, i.created_at,
      ...
      i.context, i.pain_points, i.expected_benefit, i.extra,
...
// Author, or a moderator (Project Lead / admin), can delete. Returns the blob URL.
  ...
  const rows = await sql`
    ...
    where at.id = ${attId}
      and ( at.account_id = ${accountId}
         or ${isAdmin}
         or exists (select 1 from idea_members m where m.idea_id = at.idea_id and m.account_id = ${accountId} and m.role = 'Project Lead') )
    returning url
  `;
