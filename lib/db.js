...
export async function getDashboard({ since = null, quarterStart = null } = {}) {
  ...
  // Funnel = "reached this stage or later". Every idea was submitted, so the
  // first stage is the full total — including On Hold / Declined ideas, which
  // we can't attribute to a later stage (there's no status history yet).
  ...
  const funnel = order.map((s, i) => ({
    stage: label[s],
    status: s,
    count: i === 0 ? counts.total : order.slice(i).reduce((n, st) => n + (byStatus[st] || 0), 0),
  }));
