...
async function smtpSender() {
  ...
  return {
    send: (to, from, subject, html) => transport.sendMail({ from, to, subject, html }),
...
export async function sendEmail({ subject, html, to, bcc }) {
  ...
  try {
    ...
    const results = await Promise.allSettled(
      recipients.map((addr) => sender.send(addr, from, subject, html))
    );
