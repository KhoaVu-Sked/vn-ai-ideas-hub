...
// POST /api/journey/reset → a full reset for this learner, not just course
// progress: clears course_assignments (roadmap back to not_started),
// account_tracks (un-enrolled from every track — this is what makes the Get
// Started gateway show again), user_role (seniority cleared), and
// calendar_connections (Google Calendar disconnected). Lets the whole Get
// Started flow be re-tested from scratch, not just the roadmap.
...
export async function POST() {
  try {
    ...
    const { reset, eventIds, tracksCleared, roleCleared, calendarConnectionCleared } = await resetJourney(user.uid);
    ...
    return Response.json({ reset, tracksCleared, roleCleared, calendarConnectionCleared, calendarCleared, calendarError });
  } catch (e) {
